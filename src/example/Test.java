...
/**
 * Test performs CRUD operations on Orders persisting to an Oracle NoSQL database.
 * @authors James Sutherland , Robert Greene
 */
public class Test {
    ...
    public static void main(String[] args) throws Exception {
        Test test = new Test();
        test.factory = Persistence.createEntityManagerFactory("ondb");
        
        test.testPersist();
        test.testFind();
      //Oracle NoSQL does not support secondary indexes so some getResultList type queries do not work as of the 2.1.x release.
      //test.testQuery();
        test.testUpdate();
        test.testRemove();
        test.factory.close();
    ...
    /**
     * Note - Oracle NoSQL Database (ONDB) does not currently support secondary indexes, so 
     * there is no ability to query based on value contents.  Range based queries can be 
     * performed natively within ONDB by choosing an intelligent major-minor key design. 
     */
